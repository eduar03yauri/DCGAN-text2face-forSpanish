...
from tensorflow.keras.layers import BatchNormalization,UpSampling2D,Conv2D,MaxPooling2D,Conv2DTranspose,Input,Dense,Reshape,concatenate,Flatten,Lambda,LeakyReLU,Dropout,ReLU,LayerNormalization
...
Model_path = "data/sent2vec_text2facev2_d4800ep5000.bin"
...
class DCGan(object):
	...
	def create_model(self):
		...
		generator_layer=ReLU()(generator_layer)
		generator_layer= BatchNormalization(momentum=0.2)(generator_layer) 
		...
		generator_output=Lambda(lambda x:x+1.5)(generator_layer)
		...
		img_layer2=LeakyReLU(alpha=0.2)(img_layer)

		img_layer2=Flatten()(img_layer2)
		
		img_layer2 = Dropout(0.2)(img_layer2)

		discriminator_layer=Dense(1,kernel_initializer=k_ini)(img_layer2)
		...
		self.discriminator.compile(loss='binary_crossentropy',optimizer=d_optims)
	...
	def fit(self,model_dir_path,image_label_pairs,epochs=None,batch_size=None,snapshot_dir_path=None):
		...
		for epoch in range(epochs):
			...
			if(epoch+1)%5==0 or (epoch+1)==epochs:
				...
				with h5py.File('loss3/losses_list.h5','w') as out:
					out.create_dataset("discriminator",data=np.array(d_loss_list))
					out.create_dataset("generator",data=np.array(g_loss_list))
	def generate_image_from_text(self,caption,sent2vec_model):
		#encoded_text=sent2vecs.encode(sent2vec_model,caption)
		encoded_text = sent2vec_model.embed_sentence(caption)
...
def load_normalised_img_and_cap(img_path, caption_file):
	...
	vectors = captions
	if not os.path.exists('data/sent2vec_vectors_50k.pkl'):
		sent2vec_model = sent2vec.Sent2vecModel()
		sent2vec_model.load_model(Model_path)
		vectors = sent2vec_model.embed_sentences(captions)
		with open('data/sent2vec_vector_50k.pkl','wb') as f:
			pickle.dump(vectors,f)
	else:
		with open('data/sent2vec_vectors_50k.pkl','rb') as f:
